...
def fetch_wikipedia_content(author_name):
    try:
        ...
        logging.error(f"An error occurred while fetching Wikipedia content for {author_name}: {e}")
        return ""
...
def process_file(input_file, output_file):
    ...
    for element in data:
        ...
        if isinstance(author_dblp_uri, list):
            logging.warning(f"Ignoring question with multiple DBLP URIs: {element['id']}")
            continue
        ...
        if author_name:
            ...
            logging.info(f"Author name for {author_name} written to {output_file} with empty Wikipedia content.")
            ...
            if wikipedia_content:
                ...
                logging.info(f"Wikipedia content for {author_name} updated in {output_file}")
            else:
